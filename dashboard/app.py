...
if view == "1. 이탈현황":
    ...
    with tab1:
        ...
        if not segment_df.empty:
            fig = px.bar(segment_df.head(12), x="persona", y="avg_coupon_exposure", hover_data=[col for col in ["avg_churn_probability", "avg_expected_roi"] if col in segment_df.columns], title="페르소나별 평균 혜택 노출")
            st.plotly_chart(fig, use_container_width=True)
            display_df = segment_df.copy()
            for col in ["avg_churn_probability", "avg_expected_roi"]:
                if col in display_df.columns:
                    display_df[col] = display_df[col].map(lambda x: f"{float(x):.3f}")
            _render_dataframe_with_count(display_df, label="페르소나별 혜택 노출/성과")
        else:
